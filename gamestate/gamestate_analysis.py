...
class Analysis(object):
    ...
    def robot_ball_overlap(self, robot_pos, ball_pos=None):
        ...
        return self.overlap(robot_pos, ball_pos,
    ...
    def dribbler_pos(self, team, robot_id):
        ...
        relative_pos = angle * (self.ROBOT_DRIBBLER_RADIUS + self.BALL_RADIUS)
    ...
    def dribbler_to_robot_pos(self, dribbler_pos, robot_w):
        ...
        # divide radius by 2 togo a bit closer to the ball to help make contact
        x, y = dribbler_pos - direction * (
            self.ROBOT_DRIBBLER_RADIUS + self.BALL_RADIUS / 2
        )
    ...
    def ball_in_dribbler(self, team, robot_id):
        ...
        if not self.ball_in_dribbler_single_frame(team, robot_id, first_pos):
            return False
        # look back from 0 (most recent) until big enough interval
        while (
            i < len(positions) - 1
            and positions[0][0] - positions[i][0] < MIN_TIME_INTERVAL
        ):
            ball_p = positions[i][1]
            i += 1
            if not self.ball_in_dribbler_single_frame(team, robot_id, ball_p):
                return False
        return True

    def is_position_open(self, pos, team, robot_id, buffer_dist=0):
        """
        return whether robot can be in location without colliding another robot
        """
    ...
    def get_ball_velocity(self):
        ...
        accel_dir = -midpoint_velocity / np.linalg.norm(midpoint_velocity)
        accel = accel_dir * self.BALL_DECCELERATION * time_since_midpoint
    ...
    def predict_ball_pos(self, delta_time):
        ...
        if ((velocity_initial * velocity_final) < 0).any():
            ...
            # velocity can be zero which would cause us to divide by zero if
            # we use that axis.
            if not accel[0] == 0:
    ...
    def is_shot_coming(self, team):
        ...
        GOAL_X_BUFFER = 500  # assumes shots stopping slightly short will in
    ...
    def is_ball_behind_goalie(self, team):
        ...
        ball_distance_from_goal_center = \
            np.linalg.norm(ball_pos - center_of_goal)
        return ball_distance_from_goal_center <= 600
