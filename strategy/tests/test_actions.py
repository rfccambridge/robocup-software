...
def test_pivot_with_ball():
    ...
    Passes if the robot begins to turn in the shorter path to the goal
    direction while remaining close to the ball.
    """
    ...
    assert (np.linalg.norm(goal_pos[:2] - ball_pos) < gs.ROBOT_RADIUS * CLOSE_ENOUGH_FACTOR)  # noqa
...
def test_path_find_surrounded():
    """ Tests path_find when the robot is surrounded by opponents and cannot
    find a path. Passes if the robot is not given any waypoints to try to
